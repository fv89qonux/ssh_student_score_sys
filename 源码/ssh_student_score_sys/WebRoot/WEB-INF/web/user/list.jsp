	...
	<div class="box">
		<div class="box_border">
           	...
           	<div class="box_center pt10 pb10">
               	<form action="${ctx }/user/list.html" method="post">
...
<div id="table" class="mt10">
	<div class="box span10 oh">
		...
		<jsp:include page="../common/pager.jsp">
			<jsp:param value="${ctx }/user/list.html" name="url"/>
...
$(function(){
	$(".add_").click(function(){
        ymPrompt.win({title:'添加',height:'230',width:'500',dragOut:false,iframe:true,message:'${ctx}/user/toAdd.html',handler:refresh});
    })
    
	$(".edit_").click(function(){
        var id = $(this).attr("data");
        ymPrompt.win({title:'添加',height:'230',width:'500',dragOut:false,iframe:true,message:'${ctx}/user/toUpdate.html?user.id=' + id,handler:refresh});
    })
...
function handlerDel(tp){
	if(tp=='ok'){
		$.ajax({
            type : "GET",
            url : '${ctx}/user/delete.html?user.id=' + delete_id,
            dataType : "text",
...
// 刷新页面
function refresh(){
	self.location.href="${ctx}/user/list.html";
}
</script>
</body>
</html>