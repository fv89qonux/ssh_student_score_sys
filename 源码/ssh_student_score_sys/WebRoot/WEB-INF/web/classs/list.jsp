...
$(function(){
	$(".add_").click(function(){
        ymPrompt.win({title:'添加',height:'230',width:'500',dragOut:false,iframe:true,message:'${ctx}/classs/toAdd.html',handler:refresh});
    })
    
	$(".edit_").click(function(){
        var id = $(this).attr("data");
        ymPrompt.win({title:'添加',height:'230',width:'500',dragOut:false,iframe:true,message:'${ctx}/classs/toUpdate.html?classs.id=' + id,handler:refresh});
    })
...
function handlerDel(tp){
	if(tp=='ok'){
		$.ajax({
            type : "GET",
            url : '${ctx}/classs/delete.html?classs.id=' + delete_id,
            dataType : "text",
...
// 刷新页面
function refresh(){
	self.location.href="${ctx}/classs/list.html";
}
</script>
</body>
</html>